...
class FormatterHelper {
  ...
  * Remove os sinais retornando apenas os números
...
/*
* Passa todas as letras para caixa baixa
*/
  ...
  /**
	 * Busca ID da cidade selecionada, se não encontrada, envia mensagem de erro
	 * para usar no método de FormatterHelper::bairroId
   * @author Mayra Dantas Bueno
	 */
  public static function cidadeId($cidade){

    if(!is_numeric($cidade)){
      try{
        $cidade = DB::table('cidade')->select('id')->where('nome', '=', $cidade)->first()->id;
      }catch(Exception $e){
  ...
  /**
	 * Busca ID do bairro se for de Caraguatatuba [3388] e preenchido corretamente como está no banco de dados
	 * Para salvar nome do bairro digitado e ID (se houver)
   * @author Mayra Dantas Bueno
	 */
  public static function bairroId($bairro, $cidade){

    $cidadeId = FormatterHelper::cidadeId($cidade);

    if($cidadeId == 3388 && !is_numeric($bairro)){

      try{
        $bairro = DB::table('bairro')->select('id')->where('nome', '=', $bairro)->first()->id;
      }catch(Exception $e){
        $bairro = $bairro;
      }

    }else if(is_numeric($bairro)){
      $bairro = DB::table('bairro')->select('nome')->where('id', '=', $bairro)->first()->nome;
    }
  ...
  public static function formatarDinheiro($money){
    if(strlen($money) == 3){
        $money = substr_replace($money, ',', 1, 0);
      }
    elseif(strlen($money) == 4){
        $money = substr_replace($money, ',', 2, 0);
      }
    elseif(strlen($money) == 5){
        $money = substr_replace($money, ',', 3, 0);
      }
    elseif(strlen($money) == 6){
        $money = substr_replace($money, '.', 1, 0);
        $money = substr_replace($money, ',', 5, 0);
      }
    elseif(strlen($money) == 7){
        $money = substr_replace($money, '.', 2, 0);
        $money = substr_replace($money, ',', 6, 0);
      }
    elseif(strlen($money) == 8){
        $money = substr_replace($money, '.', 3, 0);
        $money = substr_replace($money, ',', 7, 0);
      }
    elseif(strlen($money) == 9){
        $money = substr_replace($money, '.', 1, 0);
        $money = substr_replace($money, '.', 6, 0);
        $money = substr_replace($money, ',', 9, 0);
      }
    elseif(strlen($money) == 10){
        $money = substr_replace($money, '.', 2, 0);
        $money = substr_replace($money, '.', 6, 0);
        $money = substr_replace($money, ',', 10, 0);
      }
    elseif(strlen($money) > 10){
        $money = substr_replace($money, '.', 3, 0);
        $money = substr_replace($money, '.', 7, 0);
        $money = substr_replace($money, ',', 11, 0);
      }
    return 'R$ '.$money;
  }
