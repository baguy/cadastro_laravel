<?php
	class FuncionarioBuilder {
		...
		// Método com filtros de buscas da página INDEX de funcionario
		public function getSpecificRestrictions($parameters = array()) {

			// Status — apenas usuários ATIVOs
			if (array_key_exists('S_funcionario_id', $parameters)) {

				if (!empty($parameters['S_funcionario_id'])) { // Active

					$this->instance = $this->instance->where('id', $parameters['S_funcionario_id']);
			...
			if (array_key_exists('S_status_id', $parameters)) {
				...
				if (!empty($parameters['S_status_id']) && $parameters['S_status_id'] == 2 ) {
