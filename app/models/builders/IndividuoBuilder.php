<?php
	class IndividuoBuilder {
		...
		public function getSpecificRestrictions($parameters = array()) {
			...
			// Todos os usuários PRÉ-CADASTRADOS
			if (array_key_exists('S_pre_cadastro', $parameters)) {

				if (!empty($parameters['S_pre_cadastro']) && $parameters['S_pre_cadastro'] == 1) { // Active
				...
				if(!empty($parameters['S_pre_cadastro']) && $parameters['S_pre_cadastro'] == 2){

					$this->instance = $this->instance
					->whereNull('pre_cadastro');
