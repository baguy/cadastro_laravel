          ...
          <div class="tab-content">
            ...
            <div class="active tab-pane" id="chamado">
                ...
                <table class="table table-hover table-sm table-collapse">
                  ...
                  <tbody>
                    @if(isset($encerrado) && $encerrado != '')
                      @foreach( $encerrado as $key => $atendimento )
                        <tr>
                          <td>
                            ...
                            <td class="align-middle">
                             ...
                             <td class="align-middle">
                              <span class="ellipsis">{{ str_limit(ucwords(mb_strtolower($atendimento->setorFormatado(), 'UTF-8')), $limit = 25, $end = '...') }}</span>
                             </td>
                            <td class="align-middle">
                       ...
                       <div class="collapse" id="collapseAt_{{ $atendimento->id }}">
                         <div>
                           ...
                           @foreach($atendimento->assentamentos as $key => $assentamento)
                            <p><b>Assentamento {{ $key+1 }} — {{ ucwords(mb_strtolower( $atendimento->setoresFormatados($assentamento->setor), 'UTF-8')) }} — Criado em: {{ FormatterHelper::dateTimeToPtBR($assentamento->created_at) }}</b></p>
...
{{-- {{ $individuo->funcionario->setor[0]->atendimento->status_id }} --}}
